...
def config_from_annotations(annotations: Dict[str, Any]) -> Dict[str, Any]:
    """
    Reconstruct model config from asset annotations.

    Used by the API to recreate model config for inference.

    Args:
        annotations: Model annotations from registry

    Returns:
        Config dict with algorithm and hyperparameters
    """
    # Normalize algorithm name (handle both "IsolationForest" and "isolation_forest")
    raw_algorithm = annotations.get("algorithm", "isolation_forest")
    algorithm = raw_algorithm.lower().replace(" ", "_")
    ...
    if algorithm == "isolation_forest":
        if annotations.get("n_estimators"):
            hyperparameters["n_estimators"] = int(annotations["n_estimators"])
        if annotations.get("contamination"):
            hyperparameters["contamination"] = float(annotations["contamination"])
        hyperparameters["random_state"] = 42
        hyperparameters["n_jobs"] = -1
    elif algorithm == "local_outlier_factor":
        if annotations.get("n_neighbors"):
            hyperparameters["n_neighbors"] = int(annotations["n_neighbors"])
        if annotations.get("contamination"):
            hyperparameters["contamination"] = float(annotations["contamination"])
        hyperparameters["n_jobs"] = -1
...
def predict_fresh(config: Dict[str, Any], feature_set: FeatureSet) -> PredictionResult:
    """
    Train a fresh model on-the-fly and run prediction.

    Used by the API for real-time inference. This re-trains the model
    on the provided data, which works well for unsupervised anomaly
    detection where the model learns the "normal" distribution.

    Args:
        config: Model config with algorithm and hyperparameters
        feature_set: FeatureSet to train and predict on

    Returns:
        PredictionResult with predictions and scores
    """
    model = Model(config)
    _, prediction = model.train(feature_set)
    return prediction
